...
export const seedsApi = {
  ...
  async getAll(plantId: string) {
    return request<PaginatedResponse<Seed>>(`/api/plants/${plantId}/seeds`);
  },
  ...
  async getById(plantId: string, id: string) {
    return request<ApiResponse<Seed>>(`/api/plants/${plantId}/seeds/${id}`);
  },
  ...
  async create(plantId: string, data: CreateSeedInput) {
    return request<ApiResponse<Seed>>(`/api/plants/${plantId}/seeds`, {
      method: "POST",
  ...
  async delete(plantId: string, id: string) {
    return request<ApiResponse<{ message: string }>>(`/api/plants/${plantId}/seeds/${id}`, {
      method: "DELETE",
    });
  },
};
